...
def valid(data_loader, epoch, Hnet, Rnet):
    ...
    for i, data in enumerate(data_loader):
        ...
        concat_imgv = Variable(concat_img.clone(), requires_grad=False)  # 不反向传播
        cover_imgv = Variable(cover_img.clone(), requires_grad=False)
        secret_imgv = Variable(secret_img.clone(), requires_grad=False)
    ...
    if (epoch % opt.result_freq == 0) or (epoch == opt.epochs - 1):
        save_pic('valid', cover_img, stego, secret_img, secret_rev, opt.validation_pics, opt.batch_size, epoch)
    ...
    epoch_log += "learning rate: optimizerH_lr = %.8f\t optimizerR_lr = %.8f" % (
        optimizerH.param_groups[0]['lr'], optimizerR.param_groups[0]['lr']) + "\n"
    # schedulerH.get_lr()[0] schedulerR.get_lr()[0]
    epoch_log += "Hloss=%.6f\t Rloss=%.6f\t sumLoss=%.6f" % (val_hloss, val_rloss, val_sumloss) + "\n"
...
def main():
    ...
    assert dataloaders
    print_log('train and valid dataset has been prepared!', logPath)
    ...
    for epoch in range(opt.epochs):
        # 只有训练的时候才会计算和更新梯度
        train(dataloaders['train'], epoch, Hnet=modelH, Rnet=modelR)
        val_hloss, val_rloss, val_sumloss = valid(dataloaders['valid'], epoch, Hnet=modelH, Rnet=modelR)
        # 用验证集的loss来调整学习率
