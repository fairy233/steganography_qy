...
def getLoss(f, mode):
    ...
    if mode == 'train':
        temp = line_3_val[2].split('	 ')
        train_epoch.append(int(line_1_val_temp[0]))
        train_loss.append(float(temp[0]))  # 画R_loss曲线
    else:
        temp = line_3_val[2].split('	 ')
        valid_epoch.append(int(line_1_val_temp[0]))
        valid_loss.append(float(temp[0]))  # 画R_loss曲线
...
plt.title('Rloss')
plt.legend()
plt.savefig('./training1211/trainingLogs/Rloss_16_1211.jpg')
plt.show()
